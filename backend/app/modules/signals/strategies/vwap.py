...
class VWAPReversionStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        entry_zone_pct: float = 0.3,  # Entry within X% of VWAP
        trend_lookback: int = 20,  # Candles to determine trend
        min_trend_strength: float = 0.3,  # Minimum trend strength (0-1)
        require_band_touch: bool = True,  # Require touch of VWAP band
        atr_period: int = 14,
        atr_multiplier: float = 1.5,
        risk_reward_ratio: float = 2.0,
        max_distance_from_vwap_pct: float = 2.0,  # Max distance for entry
        ...
        Args:
            ...
            trend_lookback: Candles for trend detection
            min_trend_strength: Minimum trend strength to trade
            require_band_touch: Require price to touch VWAP band
            atr_period: ATR period for stop loss
            atr_multiplier: ATR multiplier for stop loss
            risk_reward_ratio: Risk/reward ratio
            max_distance_from_vwap_pct: Maximum % distance from VWAP
            no_trade_after: Time after which no new trades
        """
    ...
    def _detect_trend(self, df: pd.DataFrame) -> tuple[str, float]:
        ...
        Returns:
            Tuple of (trend_direction: "up"/"down"/"neutral", strength: 0-1)
        """
    ...
    def _check_vwap_signal(
        ...
        Args:
            df: DataFrame with VWAP and bands
        ...
        # BUY: Uptrend + pullback to VWAP/lower band + bounce
        if trend == "up":
        ...
        # SELL: Downtrend + retrace to VWAP/upper band + rejection
        if trend == "down":
            near_vwap = close >= vwap * (1 - self.entry_zone_pct / 100)
    ...
    def generate_signals(self, df: pd.DataFrame, symbol: str) -> list[SignalData]:
        ...
        # Filter to today's data only for VWAP calculation
        ...
        # Stop loss: ATR-based or opposite band
        if atr:
        ...
        # Confidence based on trend strength and VWAP position
