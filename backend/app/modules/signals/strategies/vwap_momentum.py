...
class VWAPMomentumStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        buy_threshold: int = 3,  # Minimum score for BUY
        strong_buy_threshold: int = 4,  # Score for strong BUY
        sell_threshold: int = 2,  # Maximum score for SELL
        strong_sell_threshold: int = 1,  # Score for strong SELL
        atr_period: int = 14,
        ...
        Args:
            ...
            ema_medium: Medium EMA period (default 9)
            ema_slow: Slow EMA period (default 21)
            rsi_period: RSI calculation period (default 14)
            rsi_threshold: RSI midpoint for bullish/bearish (default 50)
            volume_lookback: Period for average volume calculation (default 10)
            buy_threshold: Minimum score to generate BUY (default 3)
            strong_buy_threshold: Score for strong BUY signal (default 4)
            sell_threshold: Maximum score to generate SELL (default 2)
            strong_sell_threshold: Score for strong SELL signal (default 1)
    ...
    def _calculate_score(self, indicators: dict) -> tuple[int, list[str]]:
        ...
        Returns:
            Tuple of (score 0-5, list of bullish factors)
        """
        score = 0
        bullish_factors = []
        bearish_factors = []
    ...
    def generate_signals(self, df: pd.DataFrame, symbol: str) -> list[SignalData]:
        ...
        # Build indicators dict for output
        indicators_out = {
            "score": score,
