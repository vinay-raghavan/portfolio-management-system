...
class PriceActionVolumeSwingStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        Args:
            swing_lookback: Bars to look back for swing high/low (default 5)
            ...
            require_trend_alignment: Filter signals by EMA trend (default True)
        """
    ...
    def _is_swing_low(self, df: pd.DataFrame, idx: int, lookback: int) -> bool:
        ...
        A swing low has lower lows on both sides within the lookback period.
        """
    ...
    def _is_swing_high(self, df: pd.DataFrame, idx: int, lookback: int) -> bool:
        ...
        A swing high has higher highs on both sides within the lookback period.
        """
        if idx < lookback or idx >= len(df) - 1:
            return False
    ...
    def _detect_pattern(self, df: pd.DataFrame, idx: int) -> tuple[str | None, bool]:
        ...
        Returns:
            Tuple of (pattern_name, is_bullish)
        """
    ...
    def _find_recent_swing(
        ...
        """
        end_idx = len(df) - 2  # Start from second-to-last bar
        ...
        if buy_signal:
            ...
            signals.append(
                SignalData(
                    ...
                    f"{'near swing low' if near_swing_low else 'uptrend'}",
                )
            )
            ...
            signals.append(
                SignalData(
                    ...
                    f"{'near swing high' if near_swing_high else 'downtrend'}",
                )
            )
    ...
    def _calculate_confidence(self, df: pd.DataFrame, is_bullish: bool) -> Decimal:
        ...
        if is_bullish:
            # More bullish bars recently = higher confidence for bullish signal
            confidence = 0.4 + bullish_bars * 0.08
        else:
            # More bearish bars recently = higher confidence for bearish signal
