...
import { Clock, Check, X, AlertTriangle, Bot, ChevronRight } from 'lucide-react';
...
function PendingTradeCard({
  ...
  const isExpiringSoon = new Date(trade.expires_at).getTime() - Date.now() < 3600000; // 1 hour
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground mt-1">
            ...
            {trade.confidence_score && (
              <>
                <span>·</span>
                <span>Confidence: {trade.confidence_score.toFixed(0)}%</span>
              </>
            )}
          </div>
        </div>
